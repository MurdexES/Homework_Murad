...
namespace DictionaryService
{
    internal class Dictionary : ISerialiazation
    {
        ...
        public Dictionary CreateDictionary()
        {
            ...
            tmpDictionary.FirstLanguageName = Console.ReadLine();

            Console.WriteLine("Enter Last language on which you want to translate: ");
            tmpDictionary.LastLanguageName = Console.ReadLine();

            Console.WriteLine("Enter number of Translation Pairs: ");

            int numb = Int32.Parse(Console.ReadLine());

            for (int i = 0; i < numb; i++)
            {
                translationPairs[i] = translationPairs[i].CreateTraslationPair();
            }
